...
open(writerObj3)
    ...
    resistance_visualization(M, N, x_output1, rando1, Tco1, 0, 0, 1, 0)
    F1 = getframe(gcf1);
    writeVideo(writerObj1,F1);
    
    gcf2 = figure('units','pixels','position',[0 0 800 800]);
    resistance_visualization(M, N, x_output2, rando2, Tco2, 0, 0, 1, 0)
    F2 = getframe(gcf2);
    writeVideo(writerObj2,F2);
    
    gcf3 = figure('units','pixels','position',[0 0 800 800]);
    resistance_visualization(M, N, x_output3, rando3, Tco3, 0, 0, 1, 0)
    F3 = getframe(gcf3);
